...
if (strtoupper(substr(PHP_OS, 0, 3)) === 'WIN') {
    ...
    $uploads_dir = str_replace('file-upload', 'uploads_dir', __DIR__).'/uploads/temp';
    $sep = '/';
}
// echo $uploads_dir; die;
//$myfile = fopen("../whitelisted.json", "r") or die("Unable to open file!");
//$whitelisted = fread($myfile, filesize("../whitelisted.json"));
//$//whitelisted = json_decode($whitelisted);
//fclose($myfile);
// $httpOrigin = str_replace("https://", "", $_SERVER['HTTP_ORIGIN']);
// if (in_array($httpOrigin, $whitelisted) || strpos($httpOrigin, '.quickreviewer.com') !== false || strpos($httpOrigin, '.clavistechco.com') !== false) {
//     header('Access-Control-Allow-Origin: ' . $_SERVER['HTTP_ORIGIN']);
// } 
header('Access-Control-Allow-Origin: ' . $_SERVER['HTTP_ORIGIN']);
// else if(in_array($_SERVER['HTTP_HOST'], $whitelisted)) {
...
{
    global $uploads_dir, $sep;
    $temp_dir = $uploads_dir . $sep . $identifier;
    return  $temp_dir . $sep . $filename . '.part' . $chunkNumber;
}
...
if ($REQUEST_METHOD == "POST") {
    ...
    if (count($_POST) > 0) {
        ...
        if (!empty($_FILES)) foreach ($_FILES as $file) {
            ...
            $temp_dir = $uploads_dir . $sep . $resumableIdentifier;
            $dest_file = $temp_dir . $sep . $resumableFilename . '.part' . $resumableChunkNumber;
...
function rrmdir($dir)
{
    ...
    if (is_dir($dir)) {
        ...
        foreach ($objects as $object) {
            if ($object != "." && $object != "..") {
                if (filetype($dir . $sep . $object) == "dir") {
                    rrmdir($dir . $sep . $object);
                } else {
...
function createFileFromChunks($temp_dir, $fileName, $chunkSize, $totalSize)
{
    ...
    if ($total_files * $chunkSize >=  ($totalSize - $chunkSize + 1)) {
        ...
        if (($fp = fopen($uploads_dir . $sep . $fileName, 'w')) !== false) {
            for ($i = 1; $i <= $total_files; $i++) {
                fwrite($fp, file_get_contents($temp_dir . $sep . $fileName . '.part' . $i));
                //_log('writing chunk '.$i);
            }
            fclose($fp);
	    chmod($uploads_dir . $sep . $fileName, 0777);
        } else {
